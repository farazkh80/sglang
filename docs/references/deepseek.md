...
**Usage**: MLA optimization is enabled by default. For MLA models on Blackwell architecture (e.g., B200), the default backend is now **TRTLLM MLA**, which provides optimized decode operations. If you prefer to use FlashInfer instead, explicitly specify `--attention-backend flashinfer`. Note that TRTLLM MLA only optimizes decode operations - prefill operations (including multimodal inputs) will automatically fall back to FlashInfer MLA.
...
- FlashAttention3, FlashMLA, and Triton backend fully supports MTP usage. TRTLLM MLA falls back to FlashInfer MLA for speculative decoding operations. For FlashInfer backend (`--attention-backend flashinfer`) with speculative decoding,`--speculative-eagle-topk` parameter should be set to `1`. MTP support for the CutlassMLA backend is still under development.
